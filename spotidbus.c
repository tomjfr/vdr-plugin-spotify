...
DBusMessage* sendMethodCall(const char* objectpath, const char* busname, const char* interfacename, const char* methodname, const char *string2)
{
...
bool getStatusPlaying(void)
{
   ...
   if (!vsetupconnection())
      return false;
   DBusMessage* reply = sendMethodCall(OBJ_PATH, BUS_NAME, INTERFACE_NAME, METHOD_NAME, "PlaybackStatus");
   if (reply != NULL) {
