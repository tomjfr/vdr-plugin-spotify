...
eOSState cMenuSpotiMain::ProcessKey(eKeys Key)
{
   ...
   if (state == osUnknown) {
      ...
      case kOk:
      case kYellow:
         dsyslog("spotify: main Key Ok/Yellow");
         if (!spotiControl) {
         ...
         return osEnd;
      case kBack:
         dsyslog("spotify: main Key Back");
         return osBack;
      case kBlue:
         dsyslog("spotify: main Key Blue");
         return osEnd;
      default:
         if (Key != kNone)
            dsyslog("spotify: main unknown Key %d", Key);
      }
   }
   return state;
}
...
cString
    cPluginSpotify::SVDRPCommand(const char *Command, const char *Option,
                                 int &ReplyCode)
{
